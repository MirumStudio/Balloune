...
public class BalloonPhysics : MonoBehaviour
{
	...
	private void DragBalloon(Vector2 pTouchPosition, Vector2 pCurrentBalloonPosition)
	{
		...
		if (IsPointBeyondMaximumDistance (predictedAnchorPoint)) {
			...
			//Right now, the balloon DOES NOT go beyond its maximum distance. However, it seems as if it is constantly pulled back
			//This happens because I cannot predict the girl's movement : When she moves, the maximum distance point changes
			Debug.Log (GetDistanceBetweenParentAndPosition());
		} else if (IsBalloonAtMaximumDistance()) {
			Debug.Log ("balloon is too far");
		}
